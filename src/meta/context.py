...
class Context:
    ...
    @unroll_safe
    def bind(self, pattern, term, bound_terms):
        ...
        if isinstance(pattern, VarTerm):
            ...
            for i in range(len(pattern.items)):
                self.bind(pattern.items[i], term.items[i], bound_terms)
        elif isinstance(pattern, ListPatternTerm):
            if not isinstance(term, ListTerm):
                raise ContextError("Expected the term to be a list but was: " + term.to_string())
            for i in range(len(pattern.vars)):
                self.bind(pattern.vars[i], term.items[i], bound_terms)
            rest = term.items[len(pattern.vars):]
            self.bind(pattern.rest, ListTerm(rest), bound_terms)
            ...
            for i in range(len(term.args)):
                self.bind(pattern.args[i], term.args[i], bound_terms)

    @unroll_safe
    def resolve(self, term, bound_terms):
        ...
        if isinstance(term, VarTerm) and term.slot >= 0:
            return bound_terms[term.slot]
        elif isinstance(term, ApplTerm):
            return self.__resolve_appl(term, bound_terms)
        elif isinstance(term, ListTerm):
            return self.__resolve_list(term, bound_terms)
        else:
            return term

    @unroll_safe
    def __resolve_appl(self, term, bound_terms):
        resolved_args = []
        for arg in term.args:
            resolved_arg = self.resolve(arg, bound_terms)
    ...
    @unroll_safe
    def __resolve_list(self, term, bound_terms):
        resolved_items = []
        for i in range(len(term.items)):
            resolved_items.append(self.resolve(term.items[i], bound_terms))
